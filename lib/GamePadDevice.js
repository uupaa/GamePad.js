...
function _scanRawValues(buttons, axes) {
    ...
    // --- ANALOG ---
    if (axes.length) {
        if (axes.length === 1) { s.push("A0(" + axes[0].toFixed(2) + ")"); }
        if (axes.length === 2) { s.push("A1(" + axes[1].toFixed(2) + ")"); }
        if (axes.length === 3) { s.push("A2(" + axes[2].toFixed(2) + ")"); }
        if (axes.length === 4) { s.push("A3(" + axes[3].toFixed(2) + ")"); }
    }
    // --- DIGITAL ---
    if (axes.length) {
        if (buttons.length ===  1) { if (buttons[ 0]["pressed"]) { s.push("D0"); } }
        if (buttons.length ===  2) { if (buttons[ 1]["pressed"]) { s.push("D1"); } }
        if (buttons.length ===  3) { if (buttons[ 2]["pressed"]) { s.push("D2"); } }
        if (buttons.length ===  4) { if (buttons[ 3]["pressed"]) { s.push("D3"); } }
        if (buttons.length ===  5) { if (buttons[ 4]["pressed"]) { s.push("D4"); } }
        if (buttons.length ===  6) { if (buttons[ 5]["pressed"]) { s.push("D5"); } }
        if (buttons.length ===  7) { if (buttons[ 6]["pressed"]) { s.push("D6"); } }
        if (buttons.length ===  8) { if (buttons[ 7]["pressed"]) { s.push("D7"); } }
        if (buttons.length ===  9) { if (buttons[ 8]["pressed"]) { s.push("D8"); } }
        if (buttons.length === 10) { if (buttons[ 9]["pressed"]) { s.push("D9"); } }
        if (buttons.length === 11) { if (buttons[10]["pressed"]) { s.push("D10"); } }
        if (buttons.length === 12) { if (buttons[11]["pressed"]) { s.push("D11"); } }
        if (buttons.length === 13) { if (buttons[12]["pressed"]) { s.push("D12"); } }
        if (buttons.length === 14) { if (buttons[13]["pressed"]) { s.push("D13"); } }
        if (buttons.length === 15) { if (buttons[14]["pressed"]) { s.push("D14"); } }
        if (buttons.length === 16) { if (buttons[15]["pressed"]) { s.push("D15"); } }
        if (buttons.length === 17) { if (buttons[16]["pressed"]) { s.push("D16"); } }
        if (buttons.length === 18) { if (buttons[17]["pressed"]) { s.push("D17"); } }
    }
...
function _dumpValues(v) { // @arg Uint8Array
    ...
    // |        |        |        |        |----    | PAD_KEY_L2  (0 .. 15) NEUTRAL is 0
    // |        |        |        |        |    ----| PAD_KEY_R2  (0 .. 15) NEUTRAL is 0
...
function input_NexusPlayer(buttons, // @arg GamePadButtonObjects
    ...
    // --- ANALOG ---
    var LX  = ((axes[0] + 1) * 7 | 0) - 7; // -7 .. 0 .. 7 | L STICK X
    var LY  = ((axes[1] + 1) * 7 | 0) - 7; // -7 .. 0 .. 7 | L STICK Y
    var RX  = ((axes[2] + 1) * 7 | 0) - 7; // -7 .. 0 .. 7 | R STICK X
    var RY  = ((axes[5] + 1) * 7 | 0) - 7; // -7 .. 0 .. 7 | R STICK Y
    var L2  = (axes[3] + 1) * 7 | 0; // 0 ... 15 | L2
    var R2  = (axes[4] + 1) * 7 | 0; // 0 ... 15 | R2
...
function input_DualShock4(buttons, // @arg GamePadButtonObjects
    ...
    // D14 -> LEFT
    // D15 -> RIGHT
    // A3  -> RIGHT STICK Y (-1.00 is UP, +1.00 is DOWN)
    // --- D-PAD ---
    ...
    // --- ANALOG ---
    var RY  = ((axes[3] + 1) * 7 | 0) - 7; // -7 .. 0 .. 7 | R STICK Y

    this._currentValues.set([
    //  U, R, D, L, A, B, C, X, Y, Z, L1, R1, L3, R3, S1, S2, LX, LY, RX, RY, L2, R2
        U, R, D, L, A, B, 0, X, Y, 0, L1, R1, L3, R3, S1, S2,  0,  0,  0, RY, L2, R2
    ]);
}
