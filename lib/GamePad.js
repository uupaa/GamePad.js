...
var DEVICE_LIST = {
    ...
    "HoloPlay(Vendor:04d8Product:ef7e)": "HoloPlay",
};
var DEVICE_DATA = {
...
GamePad["prototype"] = Object.create(GamePad, {
    ...
    "length":      { "get":   GamePad_length    }, // GamePad#length():int
    "getDevice":   { "value": GamePad_getDeviceByDeviceName     }, // GamePad#getDevice(String):GamePadDevice
});
...
function GamePad_length() {
    return this._connectedDeviceCount;
}

function GamePad_getDevice(name) {
    for(var i = 0; i < this._connectedDeviceCount; i++) {
        if(this[i]["device"] === name) {
            return this[i];
        }
    }
}
